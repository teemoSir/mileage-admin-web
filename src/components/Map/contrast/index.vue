...
<script setup name="GaodeMap">
	...
	const renderCircleMarker = (dataArr) => {
        ...
        carWindow.setPosition(d.position);
        carWindow.setContent('<div style="color: blue;">' +
            '<pre><b>上报时间： </b>' + d.sendTime + '</pre>' +
            '<pre><b>经度纬度： </b>' + d.position + '</pre>' +
            '</div>');

        // 7.打开速度框
        carWindow.open(gaodeMap.value, d.position);
