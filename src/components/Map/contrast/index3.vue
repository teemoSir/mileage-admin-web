<template>
	<div class="gaodeMap" :style="{ height: `${height}px` }">
		<div :id="`container-2${mid}`" style="width: 100%; height: 100%">地图资源加载中...</div>
	</div>
</template>
...
<script setup name="GaodeMap">
	...
	// 初始化 地图
	const initMap = () => {
		gaodeMap.value = new AMap.Map(`container-2${props.mid}`, {
	...
	const markerCARmoveAnimation = (value, lineArriy) => {
		if ('move' == value) {
