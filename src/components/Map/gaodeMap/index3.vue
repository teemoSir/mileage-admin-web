<template>
	<div class="gaodeMap" :style="{ height: `${height}px` }">
		<div :id="`container-${mid}`" style="width: 100%; height: 100%"></div>
	</div>
</template>
...
<script setup name="GaodeMap">
	...
	const renderPolylineRoadValue = ref()
	const carMarkerValue = ref()
	...
	const props = defineProps({
		...
		zoom: {
			type: Number,
			default: window.innerHeight > 1000 ? 9 : 8.6
		},
		pitch: {
			type: Number,
			default: 50
		...
		mapStyle: {
			...
			default: 'normal',
			validator(value) {
				return ['normal', 'macaron', 'dark', 'fresh', 'grey', 'darkblue'].includes(value)
			}
		},
		markerCluster: {
			type: Boolean,
	...
	const initMap = () => {
		...
		if (props.center) {
			...
			gaodeMap.value.setCenter([109.794995,18.912815])
		}
	...
	const renderSimpleMarkerCAR = (dataArr, theme = 'default') => {
		const marker = new AMap.Marker({
			map: gaodeMap.value,
			position: dataArr,
			icon: car,
			offset: new AMap.Pixel(-13, -13), // 调整图片偏移
		...
		let passedPolylineL = new AMap.Polyline({
			map: gaodeMap.value,
		...
		marker.on('moving', (e) => {
			passedPolylineL.setPath(e.passedPath)
			let passedPathLg = e.passedPath.length - 1
			let lastLocation = e.passedPath[passedPathLg]
			gaodeMap.value.setCenter([lastLocation.lng, lastLocation.lat])
		})

		carMarkerValue.value = marker
	}

	const markerCARmoveAlong = (lineArr) => {
		carMarkerValue.value.moveAlong(lineArr, 1, (e) => {
			return e * window.speed > 1 ? 1 : e * window.speed
		})
	}
	const markerCARmoveAnimation = (value, lineArriy, speed) => {
		if ('move' == value) {
			// carMarkerValue.value.moveAlong(lineArriy, 1, (e) => {
			// 	return e * window.speed > 1 ? 1 : e * window.speed
			// })
			// let list = []
			// lineArriy.forEach((v) => {
			// 	list.push({ position: v, duration: 100 })
			// })
			// carMarkerValue.value.moveAlong(list)
			carMarkerValue.value.moveAlong(lineArriy, {
				duration: 200 / (speed + 1),
				autoRotation: true
			})
		} else if ('pauseMove' == value) {
			carMarkerValue.value.pauseMove()
		} else if ('resumeMove' == value) {
			carMarkerValue.value.resumeMove()
		} else if ('stopMove' == value) {
			carMarkerValue.value.stopMove()
		}
	}

	const remove = () => {
		if (null != carMarkerValue.value) {
			carMarkerValue.value.stopMove()
			carMarkerValue.value.remove()
		}
	}
	...
	const renderPolylineRoad = (dataArr, option = {}) => {
		if (renderPolylineRoadValue.value) {
			renderPolylineRoadValue.value.remove()
		}
		...
		renderPolylineRoadValue.value = polyline
	}
	...
	defineExpose({
		...
		renderCircleMarker,
		renderSimpleMarker,
		renderSimpleMarkerCAR,
		markerCARmoveAlong,
		renderAwesomeMarker,
		markerCARmoveAnimation,
		renderPolyline,
		renderPolylineRoad,
		renderCircle,
		renderPolygon,
