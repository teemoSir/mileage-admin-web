<template>
	<div class="setting-drawer-index-content">
		<div class="scrollbar">
			<h3>整体风格设置</h3>
			<div class="mileage-setting-checkbox">
				<a-tooltip v-for="(a, i) in sideStyleList" :key="i" placement="top">
					<template #title>
						<span>{{ a.tips }}</span>
					</template>
					<div :class="['mileage-setting-checkbox-item', a.style]" @click="setSideStyle(a.value)">
						<check-outlined v-if="theme === a.value" class="mileage-setting-checkbox-item-select-icon" />
					</div>
				</a-tooltip>
			</div>
			<h3>整体界面布局</h3>
			<div class="mileage-setting-checkbox">
				<a-tooltip v-for="(a, i) in layoutList" :key="i" placement="top">
					...
					<div :class="['mileage-setting-checkbox-item', a.style]" @click="layoutStyle(a.value)">
						<div class="mileage-setting-layout-menu-doublerow-inner" />
						<check-outlined v-if="layout === a.value" class="mileage-setting-checkbox-item-select-icon" />
					</div>
				</a-tooltip>
			</div>
			<a-divider />
			<div class="mb-4">
				<h3>主题色</h3>
				<div class="h-[50px]">
					<a-tooltip v-for="(item, index) in colorList" :key="index" class="mileage-setting-theme-color-colorBlock">
						<template #title>
							<span>{{ item.key }}</span>
						</template>
				...
				sideStyleList: [
					{
						...
						style: 'mileage-setting-checkbox-item-dark'
					},
					{
						tips: '亮色主题风格',
						value: ThemeModeEnum.LIGHT,
						style: 'mileage-setting-checkbox-item-light'
					},
					{
						tips: '暗黑模式',
						value: ThemeModeEnum.REAL_DARK,
						style: 'mileage-setting-checkbox-item-realdark'
					}
				],
				layoutList: [
					{
						tips: '经典',
						value: 'classical',
						style: 'mileage-setting-layout-menu-classical'
					},
					{
						tips: '双排菜单',
						value: 'doublerow',
						style: 'mileage-setting-layout-menu-doublerow'
					}
				],
				xnFormStyleOptions: [
					{
		...
		methods: {
			changeTopHanderThemeColorOpen() {
				...
				if (!this.topHeaderThemeColorOpen) {
					...
					tool.data.set('MILEAGE_TOP_HEADER_THEME_COLOR_SPREAD', false)
				}
			},
			...
			toggleState(stateName) {
				...
				tool.data.set(`MILEAGE_${toolDataName}`, this.globalStore[stateName])
			},
			// 设置整体风格主题
			setSideStyle(value) {
				this.globalStore.setTheme(value)
				tool.data.set('MILEAGE_THEME', value)
			},
			// 设置整体界面布局
			layoutStyle(value) {
				this.globalStore.setLayout(value)
				tool.data.set('MILEAGE_LAYOUT', value)
			},
			// 切换颜色
			tagColor(value) {
				tool.data.set('MILEAGE_THEME_COLOR', value)
				this.globalStore.setThemeColor(value)
			},
			// 切换表单风格
			formStyleChange(value) {
				tool.data.set('MILEAGE_FORM_STYLE', value)
...
<style type="less" scoped>
	.mileage-setting-checkbox {
		display: flex;
		margin-bottom: 20px;
	}
	.mileage-setting-checkbox-item {
		position: relative;
		width: 44px;
		height: 36px;
		margin-right: 16px;
	...
	.mileage-setting-checkbox-item::before {
		position: absolute;
		top: 0;
		left: 0;
		width: 33%;
	...
	.mileage-setting-checkbox-item::after {
		position: absolute;
		top: 0;
		left: 0;
		width: 100%;
	...
	.mileage-setting-checkbox-item-light {
		background-color: #ebeef1;
		content: '';
	}
	.mileage-setting-checkbox-item-light::before {
		background-color: #fff;
		content: '';
	}
	.mileage-setting-checkbox-item-light::after {
		background-color: #fff;
	}
	.mileage-setting-checkbox-item-dark {
		z-index: 1;
		background-color: #ebeef1;
		content: '';
	}
	.mileage-setting-checkbox-item-dark::before {
		z-index: 1;
		background-color: #001529;
		content: '';
	}
	.mileage-setting-checkbox-item-dark::after {
		position: absolute;
		top: 0;
		left: 0;
		width: 100%;
	...
	.mileage-setting-checkbox-item-realdark {
		background-color: rgba(0, 21, 41, 0.85);
	}
	.mileage-setting-checkbox-item-realdark::before {
		z-index: 1;
		background-color: rgba(0, 21, 41, 0.65);
		content: '';
	}
	.mileage-setting-checkbox-item-realdark::after {
		background-color: rgba(0, 21, 41, 0.85);
	}
	.mileage-setting-checkbox-item-select-icon {
		position: absolute;
		right: 8px;
		bottom: 8px;
		color: #1890ff;
	...
	.mileage-setting-theme-color-colorBlock {
		margin-top: 8px;
		width: 20px;
		height: 20px;
		border-radius: 2px;
	...
	.mileage-setting-layout-menu-doublerow {
		z-index: 1;
		background-color: #ebeef1;
		content: '';
	}
	.mileage-setting-layout-menu-doublerow::before {
		z-index: 1;
		width: 16%;
		background-color: #001529;
		content: '';
	}
	.mileage-setting-layout-menu-doublerow-inner {
		position: absolute;
		top: 0;
		left: 0;
		display: block;
	...
	.mileage-setting-layout-menu-doublerow::after {
		position: absolute;
		top: 0;
		left: 0;
		width: 100%;
	...
	.mileage-setting-layout-menu-classical {
		z-index: 1;
		background-color: #ebeef1;
		content: '';
	}
	.mileage-setting-layout-menu-classical::before {
		z-index: 1;
		background-color: #001529;
		content: '';
	}
	.mileage-setting-layout-menu-classical::after {
		position: absolute;
		top: 0;
		left: 0;
		width: 100%;
