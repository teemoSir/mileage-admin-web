<template>
	...
	<a-layout v-if="layout === 'classical'">
		<a-layout-sider
			...
			width="210"
		>
			<header id="mileageHeaderLogo" class="mileage-header-logo">
				<div class="mileage-header-left">
					<div class="logo-bar">
						<img class="logo" :src="sysBaseConfig.MILEAGE_SYS_LOGO" />
						<span>{{ sysBaseConfig.MILEAGE_SYS_NAME }}</span>
					</div>
				</div>
			</header>
		...
		<!-- 右侧布局 -->
		<a-layout>
			<div id="mileageHeader" class="MILEAGE-header">
				<div class="mileage-header-left" style="padding-left: 0px">
				...
				<div class="mileage-header-right">
					<user-bar />
				</div>
			</div>
			<!-- 多标签 -->
			...
			<a-layout-content class="main-content-wrapper">
				<div id="admin-ui-main" class="admin-ui-main">
					...
					<div class="main-bottom-wrapper">
						<a style="color: #a0a0a0" :href="sysBaseConfig.MILEAGE_SYS_COPYRIGHT_URL" target="_blank">{{
							sysBaseConfig.MILEAGE_SYS_COPYRIGHT
						}}</a>
					</div>
				</div>
			</a-layout-content>
	...
	<a-layout v-else-if="layout === 'doublerow'">
		<a-layout-sider v-if="!isMobile" width="80" :theme="sideTheme" :trigger="null" collapsible>
			<header id="mileageHeaderLogo" class="mileage-header-logo">
				<div class="mileage-header-left">
					<div class="logo-bar">
						<router-link to="/">
							<img class="logo" :title="sysBaseConfig.MILEAGE_SYS_NAME" src="/img/logo.jpg" />
						</router-link>
					</div>
				</div>
			</header>
			<a-menu v-model:selectedKeys="doublerowSelectedKey" :theme="sideTheme" class="mileage-doublerow-layout-menu">
				<a-menu-item
					v-for="item in menu"
					:key="item.path"
					style="
				...
				>
					...
					<div class="mileage-doublerow-layout-menu-item-fort-div">
						<span class="mileage-doublerow-layout-menu-item-fort-div-span">
							{{ item.meta.title }}
						</span>
					</div>
				</a-menu-item>
		...
		>
			<div v-if="!menuIsCollapse" id="mileageDoublerowSideTop" class="mileage-doublerow-side-top">
				<h2 class="mileage-title">{{ pMenu.meta.title }}</h2>
			</div>
			<a-menu
				v-model:collapsed="menuIsCollapse"
				v-model:openKeys="openKeys"
...
<script setup>
	...
	// 获取缓存中的菜单模块是哪个
	const menuModuleId = tool.data.get('mileage_MENU_MODULE_ID')
	let initMenu = []
	if (menuModuleId) {
		// 防止切换一个无此应用的人
	...
	const switchoverTopHeaderThemeColor = () => {
		// 界面顶栏设置颜色
		const header = document.getElementById('mileageHeader')
		topHeaderThemeColorOpen.value
			? header.classList.add('mileage-header-primary-color')
			: header.classList.remove('mileage-header-primary-color')
		// 判断是否开启了通栏
		const headerLogin = document.getElementById('mileageHeaderLogo')
		try {
			topHeaderThemeColorSpread.value
				? headerLogin.classList.add('mileage-header-logo-primary-color')
				: headerLogin.classList.remove('mileage-header-logo-primary-color')
		} catch (e) {}
		// 如果是双排菜单，吧第二排的也给渲染了
		if (layout.value === 'doublerow') {
			const mileageDoublerowSideTop = document.getElementById('mileageDoublerowSideTop')
			try {
				topHeaderThemeColorSpread.value
					? mileageDoublerowSideTop.classList.add('mileage-doublerow-side-top-primary-color')
					: mileageDoublerowSideTop.classList.remove('mileage-doublerow-side-top-primary-color')
			} catch (e) {}
		}
	}
		...
		if (moduleMenu.value.length > 0) {
			...
			if (menus.length > 0) {
				...
				if (firstMenu.menuType === 'LINK') {
					...
					// 将此模块的唯一值加入缓存
					tool.data.set('MILEAGE_MENU_MODULE_ID', id)
