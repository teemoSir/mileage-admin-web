...
const changeColor = (newPrimaryColor, theme, darkClass = 'mileage-theme-dark') => {
	return new Promise((resolve) => {
		const themeEleId = 'mileage-theme-var'
		...
		if (newPrimaryColor) {
			...
			document.body.removeAttribute('mileage-theme')
		}
		if (isRealDark) {
...
const getLocalSetting = (loadTheme) => {
	let localSetting = {}
	try {
		const theme = tool.data.get('MILEAGE_THEME')
		const themeColor = tool.data.get('MILEAGE_THEME_COLOR')
		localSetting = {
			theme,
			themeColor
		}
