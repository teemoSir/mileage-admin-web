			...
			<div class="login-form">
				<a-card>
					<div class="login-header">
						<div class="logo">
							<img :alt="sysBaseConfig.MILEAGE_SYS_NAME" src="/img/logo.jpg" />
							<label>北斗自由流收费运营管理平台</label>
						</div>
	...
	export default {
		...
		data() {
			return {
				...
				captchaOpen: config.SYS_BASE_CONFIG.MILEAGE_SYS_DEFAULT_CAPTCHA_OPEN,
				validCodeBase64: '',
				ruleForm: {
					account: 'superAdmin',
					password: '',
		...
		mounted() {
			...
			configApi.configSysBaseList().then((data) => {
				if (data) {
					...
					this.captchaOpen = formData.value.MILEAGE_SYS_DEFAULT_CAPTCHA_OPEN
					tool.data.set('MILEAGE_SYS_BASE_CONFIG', formData.value)
