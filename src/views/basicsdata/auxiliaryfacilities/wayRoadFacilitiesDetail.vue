						...
						<a-col :span="9">
							<a-form-item label="设施类型：">
								<div class="form-inside-for-content">
									<span v-for="type in facilityTypeOptions" :key="type.value">
										<template v-if="type.value == detail.type">{{ type.label }}</template>
									</span>
								</div>
							</a-form-item>
						</a-col>
...
<script setup name="WayFacilities">
	...
	const onOpen = (record) => {
		...
		//调用api数据处理
		wayFacilitiesApi.findwayCodeHainanFacilities(param).then((data) => {
			detail.value = data
		})
	}
	defineExpose({
