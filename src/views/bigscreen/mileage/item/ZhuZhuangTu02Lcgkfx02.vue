<template>
	<a-spin :spinning="loadSpinning">
		<div id="ZhuZhuangTu02Lcgkfx02" :style="{ height: `${boxHeigth}px` }"></div>
	</a-spin>
</template>
...
<script setup>
	...
	const onOpen = (record) => {
		// document.getElementById('ZhuZhuangTu02Lcgkfx02').innerHTML = ''

		loadSpinning.value = true
		boxHeigth.value = record.boxHeigth - 70

		bigscreenApi.queryBigScreenLcgkfxCc(record).then((data) => {
			if (data) {
				let Echarts = echarts.init(document.getElementById('ZhuZhuangTu02Lcgkfx02'))
				let xAxis = []
				let zjfcls = [] //总计费车次
				let zjflc = [] //总计费里程
				let pjjflc = [] //平均计费里程
				...
				const option = {
					...
					series: [
						{ yAxisIndex: 0, type: 'bar', stack: 'x', name: '总计费车次', data: zjfcls },
						{
							yAxisIndex: 0,
							type: 'bar',
							stack: 'x',
						...
						{ yAxisIndex: 1, type: 'line', name: '平均计费里程', symbol: 'none', data: pjjflc }
					]
				}
				// 绘制图表
