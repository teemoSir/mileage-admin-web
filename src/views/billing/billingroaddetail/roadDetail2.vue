							...
							<a-form-item label="收费道路类型：">
								<div class="form-inside-div-content">
									<template v-for="roadType in roadTypeOptions" :key="roadType.value">
										<a-tag color="#108ee9" v-if="roadType.value == detail.roadType">{{ roadType.label }}</a-tag>
									</template>
								</div>
							</a-form-item>
							<a-form-item label="道路编码【名称】：">
								<a-tag class="form-inside-span-content" v-if="detail.roadname != null && '' != detail.roadname"
									>{{ detail.roadCode }}【{{ detail.roadname }}】</a-tag
								>
							...
							<a-form-item label="排放水平：">
								<div class="form-inside-for-content">
									<template v-for="energyType in energyTypeOptions" :key="energyType.value">
										<a-tag color="#108ee9" v-if="energyType.value == detail.vehicleCc">{{
											energyType.label
										}}</a-tag></template
									>
								</div>
