		...
		<a-row>
			<a-col :span="24">
				<a-spin :spinning="loadSpinning1">
					<gaode-map ref="ysTrackMap" api-key="94bd11195a06ec51f597ee0fd4205332" />
				</a-spin>
			</a-col>
		</a-row>
	</a-card>
...
<script setup name="vehicleTrack">
	import tool from '@/utils/tool'
	import vehicleTrackApi from '@/api/data/vehicleTrackApi'
	import wgs84togcj02 from '@/api/billing/wgs84togcj02'
	import GaodeMap from '@/components/Map/contrast/index4.vue'
	...
	const queryTracksYs = (param) => {
		...
		vehicleTrackApi.queryTracksXny(Object.assign(param)).then((data) => {
			loadSpinning1.value = false
			if (null != data && data.length > 0) {
        let lineArr = []
        data.forEach((v) => {
          let logLat = wgs84togcj02.wgs84togcj02(v.oriLon, v.oriLat)
          // let logLat = [v.oriLon, v.oriLat]
          lineArr.push({
            position: logLat,
            radius: 5,
            fillColor: 'red',
