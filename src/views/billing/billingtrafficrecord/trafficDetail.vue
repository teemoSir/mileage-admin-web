								...
								<div @click="addRoadHighlight(item.vehicleId, item.startTime, item.endTime)">
									<div style="display: flex; align-items: center; justify-content: space-between; width: 100%">
										<span>
											...
											<span class="list-title">{{ item.roadname }}</span>
										</span>
										<a-button @click="showDetail(item.id)" style="width: 40%" type="link">查看详情</a-button>
									</div>
									<div class="time-show">{{ item.createTime }}</div>
									<div class="money-father">
										<div class="content">
											<div class="con-title">计费里程(公里)</div>
											<div style="color: #323232">{{ $TOOL.changgeKilometers(item.chargeDistance) }}</div>
										</div>
										<div class="content">
											<div class="con-title">计费费率</div>
											<div style="color: #323232">{{ $TOOL.changeRate(item.rateValue) }}</div>
										</div>
										<div class="content">
											<div class="con-title">计费金额(元)</div>
											<div style="color: #323232">{{ $TOOL.changeMoney(item.chargeAmount) }}</div>
										</div>
									</div>
								</div>
							</a-list-item>
...
<script setup name="billingtrafficDetail">
		...
		bdBillingTrafficRecordApi
			...
			.then((data) => {
				if (data) {
					...
					// 拿到起始地址和结束地址
					// bdBillingTrafficRecordApi
					// 	.bdBillingTrafficRecordLocation({
					// 		lonlat: `${data.startLon},${data.startLat}`
					// 	})
					// 	.then((data) => {
					// 		detailData.value.startAddress = data
					// 	})
					// if (data.endLon && data.endLat) {
					// 	bdBillingTrafficRecordApi
					// 		.bdBillingTrafficRecordLocation({
					// 			lonlat: `${data.endLon},${data.endLat}`
					// 		})
					// 		.then((data) => {
					// 			detailData.value.endAddress = data
					// 		})
					// }
	...
	const getBillingRoadDetailPage = () => {
		...
		bdBillingRoadDetailApi.bdBillingRoadDetailPage(listRecordParams).then((data) => {
	...
	const queryPoint = (record) => {
		...
		bdBillingTrafficRecordApi
			.bdBillingTrafficRecordQueryTracks({
