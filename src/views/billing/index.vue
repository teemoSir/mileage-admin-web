	...
	<a-card :bordered="false">
		...
		>
			<template #operator class="table-operator">
				<a-space>
					<a-button type="primary" @click="formRef.onOpen()" v-if="hasPerm('billingCycleConfigAdd')">
			...
			<template #bodyCell="{ column, record }">
				...
				<template v-if="column.dataIndex === 'action'">
					<a-space>
						<a @click="formRef.onOpen(record)" v-if="hasPerm('billingCycleConfigEdit')">配置</a>
					</a-space>
				</template>
			</template>
		</s-table>
...
<script setup name="billingcycleconfig">
	...
	// 操作栏通过权限判断是否显示
	if (hasPerm(['billingCycleConfigEdit', 'billingCycleConfigDelete'])) {
		columns.push({
			title: '操作',
			dataIndex: 'action',
			align: 'center',
	...
	const loadData = (parameter) => {
		return billingCycleConfigApi.billingCycleConfigPage(parameter).then((data) => {
			return data
		})
	}
	// 重置
	...
	// 删除
	const deleteBillingCycleConfig = (record) => {
		let params = [
			{
				pkid: record.pkid
			}
		]
		billingCycleConfigApi.billingCycleConfigDelete(params).then(() => {
			table.value.refresh(true)
		})
	}
	// 批量删除
	const deleteBatchBillingCycleConfig = (params) => {
		billingCycleConfigApi.billingCycleConfigDelete(params).then(() => {
